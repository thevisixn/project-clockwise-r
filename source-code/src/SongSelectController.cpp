...
void SongSelectController::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("set_grid_separation_h", "val"), &SongSelectController::set_grid_separation_h);
    ClassDB::bind_method(D_METHOD("get_grid_separation_h"), &SongSelectController::get_grid_separation_h);
    ADD_PROPERTY(PropertyInfo(Variant::INT, "grid_separation_h"), "set_grid_separation_h", "get_grid_separation_h");

    ClassDB::bind_method(D_METHOD("set_grid_separation_v", "val"), &SongSelectController::set_grid_separation_v);
    ClassDB::bind_method(D_METHOD("get_grid_separation_v"), &SongSelectController::get_grid_separation_v);
    ADD_PROPERTY(PropertyInfo(Variant::INT, "grid_separation_v"), "set_grid_separation_v", "get_grid_separation_v");
...
void SongSelectController::populate_grid() {
    ...
    for (int i = 0; i < mapList.size(); i++) {
        ...
        if (item) {
            ...
            item->setup_data(i, mapList[i].title, mapList[i].artist, diffText, coverTex);
...
void SongSelectController::process_map_file(String fullPath) {
    ...
    if (file.is_valid()) {
            ...
            if (dir.is_valid()) {
                ...
                while (fileName != "") {
                    if (!dir->current_is_dir()) {
                        ...
                        if (meta.coverPath.is_empty() && (ext == "png" || ext == "jpg" || ext == "jpeg")) {
                            meta.coverPath = folderPath + "/" + fileName;
                        }
                        if (meta.audioPath.is_empty() && (ext == "mp3" || ext == "ogg" || ext == "wav")) {
        ...
        if (meta.coverPath != "") {
            UtilityFunctions::print("✅ Found Map: ", meta.title, " | Cover: ", meta.coverPath);
        } else {
            UtilityFunctions::print("⚠️ Found Map: ", meta.title, " | No Cover Found!");
        }

...
private:
    ...
    double beat_to_time(double beat, double startBpm, double offset, const std::vector<TimingPoint>& timings);
...
public:
    ...
    void set_sfx_single(const Ref<AudioStream>& p_stream) { sfxSingle = p_stream; }
    ...
    void set_sfx_reverse(const Ref<AudioStream>& p_stream) { sfxReverse = p_stream; }
    Ref<AudioStream> get_sfx_reverse() const { return sfxReverse; }

    void set_sfx_overclock(const Ref<AudioStream>& p_stream) { sfxOverclock = p_stream; }
    Ref<AudioStream> get_sfx_overclock() const { return sfxOverclock; }

    void set_sfx_overheat(const Ref<AudioStream>& p_stream) { sfxOverheat = p_stream; }
    ...
    void set_hit_effect_scene(const Ref<PackedScene>& p_scene) { hitEffectScene = p_scene; }
    Ref<PackedScene> get_hit_effect_scene() const { return hitEffectScene; }

    void set_tex_background(const Ref<Texture2D>& p_tex) { texBackground = p_tex; }
    ...
    void set_sfx_all_tuned(const Ref<AudioStream>& p_stream) { sfxAllTuned = p_stream; }
    Ref<AudioStream> get_sfx_all_tuned() const { return sfxAllTuned; }

    void set_sfx_full_adjusted(const Ref<AudioStream>& p_stream) { sfxFullAdjusted = p_stream; }
    Ref<AudioStream> get_sfx_full_adjusted() const { return sfxFullAdjusted; }

    void set_sfx_tuning_finish(const Ref<AudioStream>& p_stream) { sfxTuningFinish = p_stream; }
    Ref<AudioStream> get_sfx_tuning_finish() const { return sfxTuningFinish; }

    void set_result_master_volume_db(float p_volume) { resultMasterVolumeDb = p_volume; }
    float get_result_master_volume_db() const { return resultMasterVolumeDb; }

    void set_sfx_all_tuned_volume_db(float p_volume) { sfxAllTunedVolumeDb = p_volume; }
    float get_sfx_all_tuned_volume_db() const { return sfxAllTunedVolumeDb; }

    void set_sfx_full_adjusted_volume_db(float p_volume) { sfxFullAdjustedVolumeDb = p_volume; }
    float get_sfx_full_adjusted_volume_db() const { return sfxFullAdjustedVolumeDb; }

    void set_sfx_tuning_finish_volume_db(float p_volume) { sfxTuningFinishVolumeDb = p_volume; }
    float get_sfx_tuning_finish_volume_db() const { return sfxTuningFinishVolumeDb; }
    ...
    String calculate_grade(int score, double accuracy, bool isFullCombo, bool isAllPerfect);

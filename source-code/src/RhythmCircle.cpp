...
void RhythmCircle::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("set_tex_reverse", "texture"), &RhythmCircle::set_tex_reverse);
    ClassDB::bind_method(D_METHOD("get_tex_reverse"), &RhythmCircle::get_tex_reverse);
    ...
    ClassDB::bind_method(D_METHOD("set_tex_overclock", "texture"), &RhythmCircle::set_tex_overclock);
    ClassDB::bind_method(D_METHOD("get_tex_overclock"), &RhythmCircle::get_tex_overclock);
    ...
    ClassDB::bind_method(D_METHOD("set_tex_overheat", "texture"), &RhythmCircle::set_tex_overheat);
    ClassDB::bind_method(D_METHOD("get_tex_overheat"), &RhythmCircle::get_tex_overheat);
    ...
    ClassDB::bind_method(D_METHOD("set_tex_hold_tail", "texture"), &RhythmCircle::set_tex_hold_tail);
    ClassDB::bind_method(D_METHOD("get_tex_hold_tail"), &RhythmCircle::get_tex_hold_tail);
    ...
    ClassDB::bind_method(D_METHOD("set_base_radius", "radius"), &RhythmCircle::set_base_radius);
    ClassDB::bind_method(D_METHOD("get_base_radius"), &RhythmCircle::get_base_radius);
    ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "base_radius"), "set_base_radius", "get_base_radius");

    ClassDB::bind_method(D_METHOD("set_lane_gap", "gap"), &RhythmCircle::set_lane_gap);
    ClassDB::bind_method(D_METHOD("get_lane_gap"), &RhythmCircle::get_lane_gap);
    ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "lane_gap"), "set_lane_gap", "get_lane_gap");

    ClassDB::bind_method(D_METHOD("set_visible_window", "value"), &RhythmCircle::set_visible_window);
    ClassDB::bind_method(D_METHOD("get_visible_window"), &RhythmCircle::get_visible_window);
...
void RhythmCircle::add_note(NoteData note) {
    ...
    switch (note.type) {
        case NOTE_SINGLE:
        case NOTE_HOLD:
            if (texSingle.is_valid()) s->set_texture(texSingle);
            s->set_modulate(Color(1, 1, 1));
            s->set_z_index(-1);
            break;

        case NOTE_REVERSE:
            if (texReverse.is_valid()) s->set_texture(texReverse);
        ...
        case NOTE_OVERHEAT:
            if (texOverheat.is_valid()) s->set_texture(texOverheat);
            else if (texSingle.is_valid()) s->set_texture(texSingle);
            ...
            break;
        case NOTE_ANORMALY:
            if (texAnomaly.is_valid()) s->set_texture(texAnomaly);
            else if (texSingle.is_valid()) s->set_texture(texSingle);
...
double RhythmCircle::calculate_angle_at_time(double targetTime) {
    ...
    for (const auto& note : notes) {
        ...
        if (note.type == NOTE_REVERSE) {
            ...
            if (targetTime < note.time + stopDurationSec) return accumulatedAngle;
    ...
    if (remainingTime > 0) {
        accumulatedAngle += remainingTime * localSpeed * currentDir * (2.0 * Math_PI);
    }
...
void RhythmCircle::process_input(int inputLane, double currentAudioTime) {
    ...
    if (bestNoteIndex != -1) {
        ...
        if (result != RESULT_MISS) {
            ...
            if (notes[bestNoteIndex].type == NOTE_REVERSE || notes[bestNoteIndex].type == NOTE_OVERCLOCK || notes[bestNoteIndex].type == NOTE_OVERHEAT || notes[bestNoteIndex].type == NOTE_ANORMALY) visualLane = 0.5;

            Vector2 hitPos = get_position_on_circle(angle, visualLane);
            Color effectColor = (result == RESULT_PERFECT) ? Color(1, 0.9, 0.2) : Color(0.2, 0.8, 1.0);
        ...
        if (i < noteTails.size()) {
            ...
            if (shouldShow) {
                ...
                if (isTailClipped || notes[i].endTime < audioTime) {
                    ...
                    if (cap->get_texture().is_valid()) capTexSize = cap->get_texture()->get_width();
                ...
                if (tailStartTime >= tailEndTime) {
                    ...
                    if (!isTailClipped && cap->get_texture().is_valid()) {
                         double rPx = (cap->get_texture()->get_width() / 2.0) * cap->get_scale().x;
                    ...
                    if (Math::abs(angleDiff) <= (headOffsetAngle + capOffsetAngle)) {
                        ...
                        if (grad.is_valid()) {
                            ...
                            if (isTailClipped) tailColor.a = 0.0;
                            else tailColor.a = baseAlpha;
        ...
        if (i < noteSprites.size()) {
            ...
            if (notes[i].isHit) {
                ...
                if (notes[i].finalResult == RESULT_MISS) {
                    ...
                    if (alpha <= 0) s->set_visible(false);
                }
                else {
                ...
                else if (notes[i].type == NOTE_OVERHEAT) targetColor = Color(1.0, 0, 0);
        ...
        if (notes[i].isHolding && isCorrectLane) {
            ...
            if (audioTime < releaseThreshold) {
                ...
                if (i < noteSprites.size()) {
                     ...
                     double visualLane = (double)notes[i].lane;
                     if (notes[i].type == NOTE_REVERSE || notes[i].type == NOTE_OVERCLOCK || notes[i].type == NOTE_OVERHEAT) visualLane = 0.5;
                     noteSprites[i]->set_position(get_position_on_circle(currentAngle, visualLane));
...
void RhythmCircle::spawn_hit_effect(Vector2 pos, Color color) {
    if (!hitEffectScene.is_valid()) return;

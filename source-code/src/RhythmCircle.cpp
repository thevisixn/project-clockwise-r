	...
	for (auto &note : notes) {
		if (!note.isHit && note.type == NOTE_ANORMALY) {
			if (std::abs(note.time - hitTime) < 0.001) {
				...
				// 🔥 [แก้ไขจุดที่ 2] เติมการส่งคะแนนให้ครบถ้วน
				note.finalResult = RESULT_PERFECT;
				currentCombo++;
	...
	if (GameGlobal::modAutoplay) {
		for (size_t i = 0; i < notes.size(); ++i) {
			...
			if (notes[i].type == NOTE_HOLD) {
				// บอทเริ่มกดปุ่มค้าง (เวลาเป๊ะ)
				if (!notes[i].isHolding && audioTime >= notes[i].time) {
					process_input(notes[i].lane, notes[i].time);
				}
				// บอทปล่อยปุ่ม (เวลาเป๊ะ)
				else if (notes[i].isHolding && audioTime >= notes[i].endTime) {
					process_release(notes[i].lane, notes[i].endTime);
				}
			} else {
				// บอทกดโน้ตธรรมดา (เวลาเป๊ะ)
				if (!notes[i].isHit && audioTime >= notes[i].time) {
					process_input(notes[i].lane, notes[i].time);
				}
			}
		}
